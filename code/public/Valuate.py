...
def fun_idxs_of_max_n_score(user_scores_to_all_items, top_k):
    ...
    # a = np.array([1, 2, 3, 4, 5, 6, 7, 8])    # 7个得分
    # b = np.argpartition(a, -3)[-3:]           # 输出得分最大的3个idx：b = array([5, 6, 7])
    # c = b[np.argsort(a[b])][::-1]             # 得分最大的idx在最左侧：c = array([7, 6, 5])
...
def fun_predict_auc_recall_map_ndcg_dist(
        p, best_dist, epoch, dist_source, dist_predicted, tes_masks):
    # ------------------------------------------------------------------------------------------------------------------
    # 注意：当zip里的所有子项tes_buys_masks, all_upqs维度一致时，也就是子项的每行每列长度都一样。
    #      zip后的arr会变成三维矩阵，扫描axis=1会出错得到的一行实际上是2d array，所以后边单独加一列 append 避免该问题。
    append = [[0] for _ in np.arange(len(dist_source))]

    # ------------------------------------------------------------------------------------------------------------------
    # auc，不计算。
    best_dist.best_auc = 0.
    best_dist.best_epoch_auc = epoch

    # ------------------------------------------------------------------------------------------------------------------
    # recall, map, ndcg
    at_nums = p['at_nums']          # [5, 10, 15, 20]
    ranges = range(len(at_nums))

    # 计算：所有各用户下次距离间隔得分最大的前30个。
    score_ranks = np.apply_along_axis(
        func1d=fun_idxs_of_max_n_score,
        axis=1,
        arr=dist_predicted,
        top_k=at_nums[-1])
    ...
    # 保存：recall/map/ndcg的最佳值
    for k in ranges:
        if recall[k] > best_dist.best_recall[k]:
            best_dist.best_recall[k] = recall[k]
            best_dist.best_epoch_recall[k] = epoch
        if precis[k] > best_dist.best_precis[k]:
            best_dist.best_precis[k] = precis[k]
            best_dist.best_epoch_precis[k] = epoch
        if f1scor[k] > best_dist.best_f1scor[k]:
            best_dist.best_f1scor[k] = f1scor[k]
            best_dist.best_epoch_f1scor[k] = epoch
        if map[k] > best_dist.best_map[k]:
            best_dist.best_map[k] = map[k]
            best_dist.best_epoch_map[k] = epoch
        if ndcg[k] > best_dist.best_ndcg[k]:
            best_dist.best_ndcg[k] = ndcg[k]
            best_dist.best_epoch_ndcg[k] = epoch
    del all_ranks
...
def fun_save_best_and_losses(
    ...
    fil_loss = \
        '\n\tLosses: ' + \
        '\n\t\t[{v1}]'.format(v1=', '.join(losses[0])) + \
        '\n\twds(dist2pre-linear): ' + \
        '\n\t\t[{v2}]'.format(v2=', '.join(losses[1]))
    f.write(fil_para)
    f.write(fil_best)
    f.write(fil_loss)
    f.write('\n')
