...
from public.Load_Data_by_length import fun_data_buys_masks, fun_random_neg_masks_tes
from public.Load_Data_fpmc_lr import load_data
from public.Load_Data_fpmc_lr import fun_acquire_neighbors_for_each_poi, fun_acquire_negs_tra
...
def train_valid_or_test():
    ...
    for epoch in np.arange(p['epochs']):
        ...
        # ----------------------------------------------------------------------------------------------------------
        if epoch in [p['epochs'] - 1, 99, 199, 299]:
            # 保存最优值、所有的损失值。
